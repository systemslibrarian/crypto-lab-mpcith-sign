 ...
 *    files the node under `incomplete`. The party cards and verdict panels sit
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The transcript
 *    `<pre>` blocks scroll and the party shares are long unbroken hex, so at
 *    380px much of them is clipped away. Content scrolled past the client box is not dimmed or partly
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. Where a
 *    figure draws a shape and then its label as a following `<text>`, that
